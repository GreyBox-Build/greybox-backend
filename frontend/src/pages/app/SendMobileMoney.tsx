...
const SendMobileMoney = () => {
  ...
  const groupNetworkByCountry = (data: any) => {
    ...
    if (target) {
      ...
      // Check if the selected country is Ghana
      if (target.countryName === "Ghana") {
        // Modify network names for Ghana
        return networks.map((network: any) => {
          if (network.toUpperCase() === "VODAFONE") {
            network = "TELECEL";
          } else if (network.toUpperCase() === "TIGO") {
            network = "AT (TIGO)";
          } else if (network.toUpperCase() === "AIRTEL") {
            network = "AT (AIRTEL)";
          }
          return network;
        });
